...
async function start() {
  ...
  // Remove obsolete unique indexes left by older complaint schemas.
  // The v3 schema uses the `id` field as the public complaint identifier.
  try {
    ...
    for (const index of indexes) {
      ...
      if (Object.prototype.hasOwnProperty.call(key, "complaintId")) {
        try {
          ...
          console.warn(`Could not drop legacy complaint index ${index.name}:`, dropError.message);
        }
      }
    }
  } catch (indexError) {
    console.warn("Legacy complaint index check failed:", indexError.message);
  }
