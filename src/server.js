...
app.get("/", (req, res) => {
  res.json({
    success: true,
    service: "Centurion University Complaint Tracking API v6",
    status:
...
app.get("/api/admin/stats", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      stats: { total, pending, active, completed, staff },
    });
  } catch (error) {
...
async function removeLegacyIndexes() {
  ...
  for (const index of indexes) {
    ...
    // Old schema used complaintId as a unique field. It must not remain.
    if (Object.prototype.hasOwnProperty.call(key, "complaintId")) {
      try {
        await Complaint.collection.dropIndex(index.name);
        console.log(`Dropped legacy complaint index: ${index.name}`);
      } catch (error) {
        console.warn(
          `Could not drop legacy complaint index ${index.name}:`,
          error.message
        );
      }
    }
  ...
  // Align collection indexes with the current Complaint schema.
...
async function start() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Complaint API v6 running on port ${PORT}`);
  });
}
