...
function signToken(user) {
  return jwt.sign(
    { sub: user.id || user._id?.toString(), role: user.role, email: user.email, name: user.name, department: user.department || "" },
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (
      ...
      const admin = { id: "admin", name: "Administrator", email: normalizedEmail, role: "admin", department: "Administration" };
      return res.json({ success: true, token: signToken(admin), user: admin });
    }
...
router.post("/staff", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    if (String(password).length < 8) {
      return res.status(400).json({ success: false, message: "Staff password must be at least 8 characters" });
    }
    ...
    res.status(201).json({ success: true, staff: publicUser(user) });
  } catch (error) {
