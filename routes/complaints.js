...
async function generateComplaintId() {
  ...
  do {
    id = `CU-${new Date().getFullYear()}-${crypto.randomInt(
      100000,
      1000000
    )}`;
...
function canAccessComplaint(user, complaint) {
  ...
  if (user.role === "student") {
    const studentId =
      complaint.student?._id || complaint.student;

    return (
      studentId &&
      String(studentId) === String(user.sub)
    );
  }
  ...
  if (user.role === "staff") {
    ...
    return (
      assignedId &&
      String(assignedId) === String(user.sub)
    );
  }
...
function normalizePhotoData(photo) {
  ...
  const contentType =
    photo.contentType ||
    inferContentType(photo.originalName);
...
router.get(
  ...
  async (req, res) => {
    try {
      const complaint =
        await getComplaintWithRelations(
          req.params.id,
          true
        );
...
/*
|--------------------------------------------------------------------------
| AUTHENTICATED COMPLAINT DETAILS
|--------------------------------------------------------------------------
*/
router.get(
  "/:id/details",
    ...
    try {
      ...
      if (!STATUSES.includes(status)) {
        return res.status(400).json({
          success: false,
          message:
            `Invalid status. Use: ${STATUSES.join(
              ", "
            )}`,
        });
      }
      ...
      if (
        ...
        if (
          ...
          return res.status(403).json({
            success: false,
            message:
              "Only the assigned staff member can update this complaint",
          });
        }
      }
