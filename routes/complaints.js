...
function publicComplaint(c) {
  const obj = { ...c };
  const hasPhoto = Boolean(c.hasPhoto || c.photo?.contentType || c.photo?.data);
  delete obj.photo;
  return {
    ...obj,
...
router.get("/mine", requireAuth, requireRole("student"), async (req, res) => {
  try {
    ...
    res.json({ success: true, complaints: items.map(c => publicComplaint({ ...c, hasPhoto: Boolean(c.photo?.contentType || c.photo?.data) })) });
  } catch (error) {
...
router.get("/", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      complaints: items.map(c => publicComplaint({ ...c, hasPhoto: Boolean(c.photo?.contentType || c.photo?.data) })),
...
router.get("/:id/photo", requireAuth, async (req, res) => {
  try {
    ...
    if (!complaint?.photo?.data || !complaint?.photo?.contentType) return res.status(404).json({ success: false, message: "Photo not found" });
...
router.get("/:id/details", requireAuth, async (req, res) => {
  try {
    ...
    const safe = publicComplaint({ ...complaint, hasPhoto: Boolean(complaint.photo?.contentType || complaint.photo?.data) });
